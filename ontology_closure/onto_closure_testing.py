    ...
    def extract_and_save_hierarchy(self):
        ...
        for (
            ...
            output_file = os.path.join(
                self.output_dir,
                f"{cl_id.replace(':', '_')}_hierarchy.csv",  # generate CSV file path
            )
            df.to_csv(output_file, index=False)  # save the data to CSV
...
if __name__ == "__main__":
    ...
    sparql_client = (
        SPARQLClient()
    )  # This creates an instance/object of the SPARQLClient class (sets up the connection to ubergraph). This object is used to run queries against teh ontology database.
